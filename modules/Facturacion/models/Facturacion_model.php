...
class Facturacion_model extends App_Model
{
    ...
    public function get_factura($id = '')
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            return $this->db->get(db_prefix() . 'facturacion')->row();
        }
        return $this->db->get(db_prefix() . 'facturacion')->result_array();
    }
    ...
    public function get_todas_las_facturas($exclude_paid = true)
    {
        ...
        $facturas = $this->db->get(db_prefix() . 'facturacion')->result_array();
    ...
    public function crear_factura($data)
    {
        ...
        $this->db->insert(db_prefix() . 'facturacion', $data);
    ...
    public function actualizar_factura($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'facturacion', $data);
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'facturacion');
        if ($this->db->affected_rows() > 0) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'facturacion', ['estado' => 'pagada']);
        if ($this->db->affected_rows() > 0) {
